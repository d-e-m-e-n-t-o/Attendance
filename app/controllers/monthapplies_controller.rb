  ...
  def applying_update
    month_applying_update_params.each do |id, item|
      ...
      if item[:month_request_status].present? && item[:month_request_status] != "申請中" && item[:month_check_confirm] == "1"
        ...
        flash[:danger] = "指示者確認㊞が選択されていない申請があります。" if item[:month_request_status].blank? && item[:month_check_confirm] == "1"
        flash[:warning] = "変更欄のチェックがされていない申請があります。" if item[:month_request_status] != "申請中" && item[:month_check_confirm] == "0"
