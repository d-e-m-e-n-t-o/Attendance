class Attendance < ApplicationRecord
  ...
  def edit_day_apply_items_for_required
    if (edit_day_started_at.blank? || edit_day_finished_at.blank? || edit_day_request_superior.blank?) && (edit_day_request_status == '申請中')
  ...
  def edit_day_finished_at_next_end_check_required
    if edit_day_request_status == '申請中' && edit_day_started_at.present? && edit_day_finished_at.present? && edit_day_request_superior.present? && edit_next_day == false && (edit_day_started_at > edit_day_finished_at || edit_day_started_at == edit_day_finished_at)
      errors.add(:edit_next_day,
                 'のチェックが不要、または必要な申請があります。')
    end
  end
  ...
  def edit_day_finished_at_not_next_end_not_check_required
    if edit_day_request_status == '申請中' && edit_day_started_at.present? && edit_day_finished_at.present? && edit_day_request_superior.present? && edit_next_day == true && (edit_day_started_at < edit_day_finished_at)
  ...
  def over_apply_items_for_required
    if (over_end_at.blank? || over_request_superior.blank? || over_request_status.blank?) && (over_request_status == '申請中')
  ...
  def over_apply_compare_verify_time_check_finished_at_nil_edit_day_request_status_other_than_approval
    if over_request_status == '申請中' && over_end_at.present? && over_request_superior.present? && finished_at.nil? && edit_day_request_status != '承認'
      ...
      if over_next_day == false
        errors.add(:over_next_day, 'のチェックが不要、または必要な申請があります。') if user.designated_work_end_time.change(
          year: Date.current.year, month: Date.current.month, day: Date.current.day
        ) > over_end_at || user.designated_work_end_time.change(
          year: Date.current.year, month: Date.current.month, day: Date.current.day
        ) == over_end_at
