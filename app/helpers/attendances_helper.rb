module AttendancesHelper
  ...
  def compare_verify_edit_apply_time?(id, start, finish, check)
    ...
    sta_at = if Tod::TimeOfDay.parsable?(start)
               ...
               format('%.2d',
                      attendance.edit_day_started_at.hour) + ':' + format(
                        '%.2d', attendance.edit_day_started_at.min
                      )
             end
    fin_at = if Tod::TimeOfDay.parsable?(finish)
               ...
               format('%.2d',
                      attendance.edit_day_finished_at.hour) + ':' + format(
                        '%.2d', attendance.edit_day_finished_at.min
                      )
             end
    if check == '0'
  ...
  def compare_verify_over_apply_time?(id, over, check)
    ...
    fin_time = if attendance.finished_at.nil?
                 format('%.2d',
                        user.designated_work_end_time.hour) + ':' + format('%.2d',
    ...
    fin_at = if Tod::TimeOfDay.parsable?(fin_time)
               ...
               format('%.2d',
                      attendance.finished_at.hour) + ':' + format(
                        '%.2d', attendance.finished_at.min
                      )
             end
    ove_at = if Tod::TimeOfDay.parsable?(over)
               ...
               format('%.2d',
                      attendance.over_end_at.hour) + ':' + format('%.2d',
                                                                  attendance.over_end_at.min)
             end
    if check == '0'
  ...
  def over_times(id)
    ...
    over = Tod::TimeOfDay.parse(format('%.2d',
                                       attendance.over_end_at.hour) + ':' + format('%.2d', attendance.over_end_at.min))
    ove_time = (over - finish)
    if finish == over && attendance.over_next_day == true
      (format('%.2d',
              ove_time.hour + 24) + ':' + format('%.2d',
                                                 ove_time.min))
    else
